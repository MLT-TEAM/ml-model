...
BEGIN
    ...
    FROM (
        ...
        ORDER BY feature_vector <-> query_cube
        LIMIT k
    ) AS nearest;
...
create or replace function knn_predict_test_samples_removed(
  p_map real,
  p_tps real,
  p_force real,
  p_power real,
...
BEGIN
    ...
    FROM (
        ...
        FROM df_normalized
        WHERE fault IS NOT NULL
        and df_normalized.id not in (select id from random_samples)    
        ORDER BY feature_vector <-> query_cube 
        LIMIT k 
    ) AS nearest;
